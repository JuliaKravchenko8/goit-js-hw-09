...
const options = {
    ...
    onClose (selectedDates) {
        ...
        if (options.defaultDate.getTime() > selectedDates[0].getTime()) {
            (0, _notiflixNotifyAio.Notify).failure("Please chose a date in the future");
            return;
        }
...
function convertMs(ms1) {
    ...
    const day = hour * 24;
    // Remaining days
    const days = addLeadingZero(Math.floor(ms1 / day));
    // Remaining hours
    const hours = addLeadingZero(Math.floor(ms1 % day / hour));
    // Remaining minutes
    const minutes = addLeadingZero(Math.floor(ms1 % day % hour / minute));
    // Remaining seconds
    const seconds = addLeadingZero(Math.floor(ms1 % day % hour % minute / second));
    return {
        days,
        hours,
        minutes,
...
function onStartClick() {
    intervalId = setInterval(()=>{
        ...
        if (ms < 1000) clearInterval(intervalId);
    }, 1000);
}
...
},{"notiflix/build/notiflix-notify-aio":"eXQLZ","flatpickr":"llQu5","flatpickr/dist/flatpickr.min.css":"eVN6V","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"eXQLZ":[function(require,module,exports) {
var global = arguments[3];
/*
