...
function createPromise(position, delay) {
    return new Promise((resolve, reject)=>{
        ...
        setTimeout(()=>{
            if (shouldResolve) resolve((0, _notiflixDefault.default).Notify.success(`✅ Fulfilled promise ${position} in ${delay}ms`));
            else reject((0, _notiflixDefault.default).Notify.failure(`❌ Rejected promise ${position} in ${delay}ms`));
        }, delay);
    });
}
